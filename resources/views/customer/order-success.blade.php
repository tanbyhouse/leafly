            ...
            <div class="p-8">
                <div class="border-b pb-6 mb-6">
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <p class="font-bold text-lg text-leafly-dark">{{ $pesanan->nomor_pesanan }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500">Tanggal Pesanan:</span>
                            <p class="font-bold">{{ $pesanan->created_at->format('d M Y, H:i') }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500">Status Pembayaran:</span>
                            <span class="px-3 py-1 rounded-full text-xs font-bold
                                {{ $pesanan->status_pembayaran === 'dibayar' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                ...
                <div class="border-t pt-6 mb-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="font-bold">{{ $pesanan->alamat->nama_penerima }}</p>
                        <p class="text-sm text-gray-600">{{ $pesanan->alamat->telepon }}</p>
                        <p class="text-sm text-gray-600 mt-2">{{ $pesanan->alamat->alamat_lengkap }}</p>
                        <p class="text-sm text-gray-600">
                            {{ $pesanan->alamat->kota->nama_kota ?? '' }},
                            {{ $pesanan->alamat->provinsi->nama_provinsi ?? '' }}
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">Rp {{ number_format($pesanan->pajak, 0, ',', '.') }}</span>
                        </div>
                        @if($pesanan->diskon > 0)
                        <div class="flex justify-between text-green-600">
                            <span>Diskon</span>
                            <span class="font-medium">- Rp {{ number_format($pesanan->diskon, 0, ',', '.') }}</span>
                        </div>
                        @endif
                ...
                <div class="mt-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                    <p class="font-bold text-yellow-800 mb-1">Metode Pembayaran: {{ strtoupper($pesanan->metode_pembayaran) }}</p>
                    @if($pesanan->metode_pembayaran === 'transfer')
                        <p class="text-sm text-yellow-700">Silakan lakukan transfer ke rekening yang akan dikirimkan via email/WhatsApp</p>
                    @else
                        <p class="text-sm text-yellow-700">Siapkan uang pas saat kurir datang mengantarkan pesanan Anda</p>
                    @endif
                </div>
